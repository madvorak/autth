...
inductive RewritesLeftmost (r : ContextFreeRule T N) : List (Symbol T N) → List (Symbol T N) → Prop
  ...
  /-- There are terminals further left that the replaced symbol -/
...
`g.ProducesLeftmost u v` means that one application of a rule from `g` to the leftmost nonterminal of
`u` send `u` to `v`. -/
def ProducesLeftmost (g : ContextFreeGrammar.{uN} T) (u v : List (Symbol T g.NT)) : Prop :=
  ∃ r ∈ g.rules, r.RewritesLeftmost u v

/-- Given a context-free grammar `g` and strings `u` and `v`
`g.DerivesLeftmost u v` means that `g` can transform `u` to `v` in some number of rewriting steps,by
applying the transformation always to the leftmost symbol of `u` -/
...
`g.GeneratesLeftmost s` means that `g` can transform its initial nonterminal to `s` in some number of
rewriting steps, always applied to the leftmost symbol -/
...
theorem leftmost_language_eq_language : g.language = g.leftmost_language :=
  by sorry
